...
class CommitPanelView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        repo_owner = "nome_proprietario"  # modifica con il nome del proprietario
        repo_name = "nome_repository"      # modifica con il nome del repository
        github_api_url = f"https://api.github.com/repos/{repo_owner}/{repo_name}/commits"
        try:
            r = requests.get(github_api_url, timeout=5)
            if r.status_code == 200:
                commits = r.json()
                context["commits"] = commits[:5]  # prendi gli ultimi 5 commit
            else:
